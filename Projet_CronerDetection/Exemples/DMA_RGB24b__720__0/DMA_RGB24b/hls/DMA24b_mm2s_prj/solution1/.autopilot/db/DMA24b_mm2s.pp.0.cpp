...
# 2 "<built-in>" 2
# 1 "../src/DMA24b_mm2s.cpp" 2
# 1 "../src/DMA24b_mm2s.h" 1



# 1 "../src/DMA24b_mm2s_utils.h" 1
...
# 8 "../src/DMA24b_mm2s_utils.h" 2


# 1 "../src/generics_n_options.h" 1
# 11 "../src/DMA24b_mm2s_utils.h" 2


typedef ap_uint<128> axi_data_slv;
typedef ap_uint<128*3> mm_group_t;
typedef ap_uint<(3*8)> video_pixel_t;

typedef ap_axiu<(3*8),1,0,0> video_stream_element_out;
typedef hls::stream<video_stream_element_out> video_stream_out;
typedef hls::stream<axi_data_slv,256> mm_fifo_t;

typedef ap_uint<12> param_type_0;
typedef ap_uint<1> param_type_1;
# 32 "../src/DMA24b_mm2s_utils.h"
const int BEATS_PER_LINE_c = (640 / 16) * 3;
const int MM_LINE_STRIDE_c = (640 / 16) * 3;
const int DDR_SIZE = MM_LINE_STRIDE_c * 480;
# 55 "../src/DMA24b_mm2s_utils.h"
inline video_pixel_t DMA24b_mm2s_get_pixel(const mm_group_t &grp, int p){
#pragma HLS INLINE
 video_pixel_t px;

 ap_uint<(3*8)> raw = grp.range((3*8)*(p+1)-1, (3*8)*p);
 px.range(3*8 -1, 2*8) = raw.range( 8 -1, 0);
 px.range(2*8 -1, 8) = raw.range(2*8 -1, 8);
 px.range( 8 -1, 0) = raw.range(3*8 -1, 2*8);
...
#pragma HLS INLINE
 ...
 ap_uint<(3*8)> raw;
 raw.range( 8 -1, 0) = px.range(3*8 -1, 2*8);
 raw.range(2*8 -1, 8) = px.range(2*8 -1, 8);
 raw.range(3*8 -1, 2*8) = px.range( 8 -1, 0);
 grp.range((3*8)*(p+1)-1, (3*8)*p) = raw;
...
# 5 "../src/DMA24b_mm2s.h" 2
# 17 "../src/DMA24b_mm2s.h"
__attribute__((sdx_kernel("DMA24bUnit_mm2s", 0))) void DMA24bUnit_mm2s (video_stream_out &STR_video_out, param_type_0 image_w, param_type_0 image_h, volatile axi_data_slv *BAR_image_in);


void DMA24b_mm2s (video_stream_out &STR_video_out, param_type_0 image_w, param_type_0 image_h, volatile axi_data_slv *image_in);
# 2 "../src/DMA24b_mm2s.cpp" 2
...
void DMA24b_mm2s_fetch(volatile axi_data_slv* mm_buffer, mm_fifo_t &fifo, int img_w, int img_h){
 ...
 const int beats_per_line = (img_w/16)*3;
 const int nb_burst = beats_per_line/40;
 line_loop: for (int j=0; j<img_h; j++){
  burst_loop: for (int i=0; i<nb_burst; i++){
   beat_loop: for (int b=0; b<40; b++){
#pragma HLS PIPELINE II = 1
 data_mm_buffer = mm_buffer[i*40 + b + j*MM_LINE_STRIDE_c];
    fifo << data_mm_buffer;
   }
  }
 }
}
# 34 "../src/DMA24b_mm2s.cpp"
void DMA24b_mm2s_streamout(video_stream_out &STR_video_out, mm_fifo_t &fifo, int img_w, int img_h){
 video_stream_element_out video_elt_out;
 mm_group_t grp;
 const int last_i = img_w/16 - 1;
 line_loop: for(int j = 0; j < img_h; j++){
  group_loop: for(int i = 0; i <= last_i; i++){
#pragma HLS PIPELINE II = 16
 read_loop: for(int b = 0; b < 3; b++){
#pragma HLS UNROLL
 grp.range(128*(b+1)-1, 128*b) = fifo.read();
   }
   px_loop: for(int p = 0; p < 16; p++){
#pragma HLS UNROLL
 video_elt_out.data = DMA24b_mm2s_get_pixel(grp, p);
    video_elt_out.keep = -1;
    video_elt_out.strb = -1;
    video_elt_out.last = ((i == last_i) && (p == 16 -1)) ? 1 : 0;
    video_elt_out.user = ((j == 0) && (i == 0) && (p == 0)) ? 1 : 0;
...
__attribute__((sdx_kernel("DMA24bUnit_mm2s", 0))) void DMA24bUnit_mm2s (video_stream_out &STR_video_out, param_type_0 image_w, param_type_0 image_h, volatile axi_data_slv *image_in){
#pragma HLS TOP name=DMA24bUnit_mm2s
# 58 "../src/DMA24b_mm2s.cpp"
...
#pragma HLS INTERFACE m_axi port=image_in depth=DDR_SIZE offset=direct max_read_burst_length=64
